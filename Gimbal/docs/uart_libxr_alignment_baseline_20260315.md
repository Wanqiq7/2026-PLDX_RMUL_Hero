...
- 禁止对局部变量发起 `PENDING` 读请求
- 禁止在已有 pending read 未完成时重复提交新的 `USARTRead`
- 协议层重同步逻辑必须留在任务上下文，不得塞回 HAL 回调
- 第二阶段必须先提供“只消费当前 FIFO、永不挂起”的安全读取接口，再让协议层建立状态机

## 第二阶段接口约束

- BSP 新接口只允许消费当前软件 FIFO 中已存在的数据
- 数据不足时必须立即返回，不保存调用方提供的缓冲指针
- 新接口不得改变现有 `USARTRead()` 的 `BLOCK/CALLBACK/POLLING` 语义
- `vtm_input` 只能基于持久化 parser context 做流式找头、拼帧、CRC 与自恢复
