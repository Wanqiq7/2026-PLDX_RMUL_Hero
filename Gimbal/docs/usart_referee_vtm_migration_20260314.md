...
- `Gimbal` 的图传链路输入 `vtm_input` 当前已改为“BSP 非挂起 FIFO 消费接口 + 任务上下文流式拼帧解析”路径，不再走 legacy fixed-length callback
...
- 第二阶段协议层约束：不得对局部变量发起 pending read；不得在 pending read 未完成时重复提交新的 `USARTRead`
