...
static const uint16_t vision_crc16_table[256] = {
...
Vision_Recv_s *VisionInit(Vision_Init_Config_s *config) {
  ...
  if (vision_usb_rx_ptr == NULL) {
    LOGERROR("[vision] VisionInit: USB init failed!");
    return NULL;
  }

  vision_initialized = 1;
  LOGINFO("[vision] USB VCP vision link initialized (reload=%u).", daemon_config.reload_count);
...
void VisionSendIMUPacket(
  ...
  vision_tx_frame.crc16 = VisionCRC16(
    (uint8_t *)&vision_tx_frame, sizeof(vision_tx_frame) - sizeof(vision_tx_frame.crc16));
...
static void VisionTryParse(void) {
  while (vision_rx_len >= sizeof(Vision_To_Gimbal_t)) {
    ...
    uint16_t crc = VisionCRC16(
      (uint8_t *)&packet, sizeof(packet) - sizeof(packet.crc16));
    if (crc != packet.crc16) {
      vision_rx_head = (vision_rx_head + 1) % VISION_RX_BUF_LEN;
      vision_rx_len--;
      continue;
    ...
    vision_rx_head = (vision_rx_head + sizeof(Vision_To_Gimbal_t)) % VISION_RX_BUF_LEN;
