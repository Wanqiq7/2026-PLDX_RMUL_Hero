...
// DM-IMU-L1 说明书 V1.2 的 CRC16 查找表与实现（poly 0x1021, init 0xFFFF）
inline constexpr uint16_t DM_CRC16_TABLE[256] = {
...
inline uint16_t Get_CRC16(const uint8_t * ptr, uint16_t len)
{
  uint16_t crc = 0xFFFF;
  for (uint16_t i = 0; i < len; ++i) {
    uint8_t index = static_cast<uint8_t>((crc >> 8) ^ ptr[i]);
    crc = static_cast<uint16_t>((crc << 8) ^ DM_CRC16_TABLE[index]);
  }
  return crc;
}
