                               ...
                               uint8_t in_isr)
{
    ...
    if (USARTAsyncWritePortEmptySize(port) < send_size || port->queue_info.size >= port->queue_info.capacity)
        return USART_STATUS_FULL;
...
USART_Status_e USARTAsyncWritePortLoadNext(USART_Write_Port_s *port, uint8_t in_isr)
{
    ...
    if (USARTByteQueuePopBatch(&port->queue_data, target_buffer, request.data_size, in_isr) !=
        USART_STATUS_OK)
        return USART_STATUS_EMPTY;
