...
#define YAW_CHASSIS_ALIGN_ECD                                                  \
  5646 // 云台和底盘对齐指向相同方向时的电机编码器值,若对云台有机械改动需要修改
#define YAW_ECD_GREATER_THAN_4096                                              \
  1 // ALIGN_ECD值是否大于4096,是为1,否为0;用于计算云台偏转角度
#define PITCH_HORIZON_ECD                                                      \
  0 // 云台处于水平位置时编码器值,若对云台有机械改动需要修改
...
#define REDUCTION_RATIO_LOADER 51.0f // 英雄需要修改为3508的51.0f
#define NUM_PER_CIRCLE 6             // 拨盘一圈的装载量
// 机器人底盘修改的参数（⭐ 单位统一为 m，米）
#define WHEEL_BASE 0.412f  // 纵向轴距(前进后退方向)，单位:m
#define TRACK_WIDTH 0.412f // 横向轮距(左右平移方向)，单位:m
