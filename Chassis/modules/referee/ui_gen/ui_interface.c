...
static unsigned char calc_crc8(unsigned char *pchMessage, unsigned int dwLength) {
...
static uint16_t calc_crc16(uint8_t *pchMessage, uint32_t dwLength) {
  ...
  while (dwLength--) {
    ...
    wCRC = (uint16_t)((wCRC >> 8) ^
                      wCRC_Table[(wCRC ^ (uint16_t)chData) & 0x00ff]);
  }
...
#define DEFINE_FRAME_PROC(num, id)                                              \
  void ui_proc_##num##_frame(ui_##num##_frame_t *msg) {                         \
    msg->header.SOF = 0xA5;                                                     \
    msg->header.length = (uint16_t)(6U + 15U * (num));                          \
    msg->header.seq = seq++;                                                    \
    msg->header.crc8 = calc_crc8((uint8_t *)msg, 4);                            \
    msg->header.cmd_id = 0x0301;                                                \
    msg->header.sub_id = (id);                                                  \
    msg->header.send_id = (uint16_t)ui_self_id;                                 \
    msg->header.recv_id = (uint16_t)(ui_self_id + 256);                         \
    msg->crc16 = calc_crc16((uint8_t *)msg, (uint32_t)(13U + 15U * (num)));     \
  }
...
void ui_proc_string_frame(ui_string_frame_t *msg) {
  ...
  msg->header.send_id = (uint16_t)ui_self_id;
  msg->header.recv_id = (uint16_t)(ui_self_id + 256);
  msg->option.str_length =
      (uint32_t)strnlen(msg->option.string, sizeof(msg->option.string));
...
static int ui_pick_pack_size(int remaining) {
  if (remaining >= 7) {
    return 7;
  }
  if (remaining >= 5) {
...
void ui_scan_and_send(const ui_interface_figure_t *ui_now_figures,
  ...
  if (total_figures > 0) {
    ...
    for (int i = 0, pack_count = 0, pack_size = 0; i < total_figures; i++) {
      if (ui_dirty_figure[i] == 0U) {
        continue;
      }

      if (pack_count == 0) {
        pack_size = ui_pick_pack_size(pending_graphics);
      }

      if (pack_size == 7) {
        _ui_7_frame.data[pack_count] = ui_now_figures[i];
      } else if (pack_size == 5) {
        _ui_5_frame.data[pack_count] = ui_now_figures[i];
      } else if (pack_size == 2) {
        _ui_2_frame.data[pack_count] = ui_now_figures[i];
      } else {
        _ui_1_frame.data[pack_count] = ui_now_figures[i];
      }
