...
// 2. 周期更新（在底盘任务中按当前流程调用）
PowerUpdateRefereeOnline(power_ctrl, referee_online, robot_level);
PowerUpdateRefereeData(power_ctrl, power_limit_w, power_buffer, chassis_power_w);
...
```c
// 当前主线建议在 ChassisTask 中完成状态更新与任务调用
...
uint8_t cap_online = status->cap_online;
uint8_t rls_enabled = status->rls_enabled;
```
