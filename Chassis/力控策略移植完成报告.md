...
> 2026-04-03 架构补充：
> 为了让 `Chassis/application/chassis/chassis.c` 回到应用编排层，
> 力控控制核已迁移到 `Chassis/modules/algorithm/chassis_force_control.c`，
> 双板 CAN 桥接已迁移到 `Chassis/modules/can_comm/chassis_can_link.c`。
> 本文中的函数名称仍然有效，但其归属文件已与最初版本不同。
...
### 2. 力控PID控制器 (`chassis_force_control.c`)
...
### 3. 核心函数实现（当前位于 `modules/algorithm/chassis_force_control.c`）
...
完整的力控流程：
```c
// 1. 通过独立力控模块执行速度→力→电流链路
ChassisForceControlSetCommand(...);
